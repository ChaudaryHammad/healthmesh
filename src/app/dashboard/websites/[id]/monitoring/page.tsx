...
export default async function WebsiteMonitoringPage({ params }: Props) {
  ...
  const [website, entitlements, user] = await Promise.all([
    prisma.website.findFirst({
      ...
      include: {
        monitor: {
          include: {
            // Enough history for 24h charting at 1-minute intervals is not
            // feasible in one payload; 300 covers 5h at 1m and 3+ days at 15m.
            checks: {
  ...
  const totalCheckCount = monitorId
    ? await prisma.uptimeCheck.count({ where: { monitorId } })
    : 0;
  ...
  const monitor: MonitorDetail | null = website.monitor
    ? {
        ...
        createdAt: website.monitor.createdAt.toISOString(),
      }
    : null;
